...
## 8.1 빈 생명주기 콜백
- 데이터베이스 커넥션 풀, 네트워크 소켓처럼 애플리케이션 시작 시점에 필요한 연결을 미리하고 종료 시점에 연결을 모두 종료하는 작업 → 객체의 초기화와 종료 작업 필요
- 스프링 빈은 객체 생성하고 의존관계 주입이 다 끝난 다음에 필요한 데이터 사용 준비가 완료됨
- 따라서 **초기화 작업은 의존관계 주입이 모두 완료되고 난 다음 호출**해야함 그럼 의존관계 주입 완료 시점은 언제?
  - 스프링이 의존관계 주입이 완료되면 스프링 빈에게 **콜백 메서드 통해 초기화 시점을 알려주는 기능 제공**함
  - 스프링 컨테이너가 종료되기 직전에 **소멸 콜백**을 줌
- 스프링 빈의 이벤트 생명주기
