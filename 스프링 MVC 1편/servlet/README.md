...
### 프론트 컨트롤러 V4 - 단순하고 실용적인 컨트롤러
- 컨트롤러 인터페이스를 구현할 때 항상 ModelView 생성/반환하는 과정이 번거로움
- V4에서는 컨트롤러는 ModelView 객체를 반환하지 않고 `viewName` 뷰 이름만 반환
- 기존 구조에서 모델을 파라미터로 넘기고 뷰의 논리 이름을 반환

### 프론트 컨트롤러 V5 - 유연한 컨트롤러
#### 어댑터 패턴
- 현재 프론트 컨트롤러 v4까지는 한가지 방식의 컨트롤러 인터페이스만 사용 가능
- 어댑터 패턴을 사용해 프론트 컨트롤러가 다양한 방식의 컨트롤러를 처리할 수 있게 함
- 핸들러 어댑터
  - 다양한 종류의 컨트롤러를 호출할 수 있게 어댑터 역할
- 핸들러
  - 컨트롤러의 이름을 넓은 범위인 핸들러로 변경
...
- `ModelView handle(HttpServletRequest request, HttpServletResponse response,
  ...
  - 실제 컨트롤러가 ModelView를 반환하지 못하면 어댑터가 대신 생성해서라도 반환해햐함
  - 이전엔 프로튼 컨트롤러가 컨트롤러를 호출했으나 지금은 어댑터가 호출

### 정리
- V1 : 기존 구조 유지하면서 `프론트 컨트롤러 도입`
- V2 : 단순 반복되는 `뷰 로직 분리`
- V3 : `Model 추가`, 서블릿 종속성 제거, 뷰 이름 중복 제거
- V4 : 구현 입장에서 ModelView 직접 생성해 반환하지 않게 인터페이스 제공
- V5 : `어댑터`를 추가해 프레임워크를 유연하고 확정성 있게 설계